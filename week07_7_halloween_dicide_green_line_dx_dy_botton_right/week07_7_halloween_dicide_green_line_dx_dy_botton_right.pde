...
void draw()
{
  ...
  for (int i=0; i<pt.size()-1; i++){ // -1下面要加回來
    ...
    if (p2.x - p.x > 0 && p2.y - p.y > 0) botton_right++;
  }
  if (dpx>100 && dpy<40) stroke(255, 0, 0); // horizonal line (red)
  else if (dpx<40 && dpy>100) stroke(0, 0, 255); // vertical line (blue)
  else if (botton_right>30) stroke(0, 255, 0); // botton_right (green)
  else stroke(0); // (black)
  for (int i=0; i<pt.size()-1; i++){ // -1下面要加回來
    PVector p = pt.get(i), p2 = pt.get(i+1); // +1對消
    line(p.x, p.y, p2.x, p2.y); // ellipse(p.x, p.y, 3, 3);
